...
@loader_blueprint.route('/post')   # блюпринт страницы загрузки постов
def post_page():
    return render_template('post_form.html')


@loader_blueprint.route('/post', methods=['POST'])   # блюпринт загрузки постов
def add_post_page():
    picture = request.files.get('picture')
    content = request.form.get('content')

    if not picture or not content:
        return 'Нет картиники или текста'
    ...
    try:
        ...
        return 'Не валидный файл Posts.json'
